...
// public	##	##	##	##	##	##	##	##	##	##	##	##	##	##	##	##	##	##	##	##
    ...
    private static record EventDefinition<T extends Event> (EventHandler<? super T> handler, EventType<T> type,
            EventTarget target) {
    }

    public static Subscription setzeValidierungStyle(Node node, Validierung validierung) {
        updateStyleClass(node, validierung.isValidProperty().get());
        return EasyBind.includeWhen(node.getStyleClass(), CSS_EINGABE_FEHLER,
                validierung.isValidProperty().not());
    }

    private static void updateStyleClass(Node node, boolean isValid) {
        if (isValid && node.getStyleClass().contains(CSS_EINGABE_FEHLER)) {
            node.getStyleClass().remove(CSS_EINGABE_FEHLER);
        } else if (!isValid && !node.getStyleClass().contains(CSS_EINGABE_FEHLER)) {
    ...
    public static Subscription setzeValidierungMeldungen(Node node, Validierung validierung) {
        setzeValidierungMeldungen(node, validierung, validierung.isValidProperty().get());
        return EasyBind.subscribe(validierung.isValidProperty(), valid -> {
            setzeValidierungMeldungen(node, validierung, valid.booleanValue());
        });
    }

    private static void setzeValidierungMeldungen(Node node, Validierung validierung, boolean isValid) {
    ...
    private static void addEventDefinition(Node node, EventHandler<? extends Event> handler, EventType<?> type,
            EventTarget target) {
    ...
    public static Subscription setzePlatzhalter(TextInputControl eingabefeld, SimpleValidierung validierung) {
        updatePlatzhalter(eingabefeld, validierung, validierung.isValidProperty().get());
        return EasyBind.subscribe(validierung.isValidProperty(), valid -> {
            updatePlatzhalter(eingabefeld, validierung, valid.booleanValue());
        });
    }

    private static void updatePlatzhalter(TextInputControl eingabefeld, SimpleValidierung validierung,
            boolean isValid) {
        if (isValid) {
    ...
    private static void updatePlatzhalter(CheckBox eingabefeld, SimpleValidierung validierung, boolean isValid) {
        if (isValid) {
            eingabefeld.setTooltip(null);
        } else {
            Optional<String> fehler = validierung.getHighestErrorMessage();
