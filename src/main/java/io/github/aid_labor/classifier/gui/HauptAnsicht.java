...
//	* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
    ...
    public HauptAnsicht(ProgrammDetails programm, HostServices rechnerService, Runnable easterEgg) {
        ...
        var scrollContainer = new ScrollPane(ribbonKomponente.getRibbon());
        scrollContainer.setVbarPolicy(ScrollBarPolicy.NEVER);
        scrollContainer.setHbarPolicy(ScrollBarPolicy.AS_NEEDED);
        scrollContainer.setMaxWidth(Double.MAX_VALUE);
        EasyBind.subscribe(scrollContainer.widthProperty(), w -> {
            scrollContainer.setFitToWidth(w.intValue() > 1100);
        });
        Platform.runLater(() -> scrollContainer.prefViewportHeightProperty()
                .bind(new When(((ScrollPaneSkin) scrollContainer.getSkin()).getHorizontalScrollBar().visibleProperty())
                        .then(ribbonKomponente.getRibbon().heightProperty())
                        .otherwise(Region.USE_PREF_SIZE)));

        var top = new VBox(menueAnsicht.getMenueleiste(), scrollContainer);
        hauptInhalt.setTop(top);
